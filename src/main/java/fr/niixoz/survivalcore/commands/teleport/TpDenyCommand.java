...
public class TpDenyCommand extends AbstractCommand {
    ...
    public boolean executeCommand(Player player, Command command, String s, String[] args) throws Exception {
        ...
        if(args.length == 0) {
            player.sendMessage("§6[ §eTéléportation §6] §eUsage: §6/tpdeny <joueur>");
            return true;
        }
        ...
        task.deny();
        player.sendMessage("§6[ §eTéléportation §6] §eVous avez refusé la demande de téléportation de §6" + task.getPlayer().getName() + "§e.");
